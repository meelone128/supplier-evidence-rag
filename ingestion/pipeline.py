...
def embed_texts_with_client(
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if cfg.embeddings.dimensions:
            kwargs["dimensions"] = cfg.embeddings.dimensions
...
def run_ingestion(cfg: RootConfig) -> str | None:
    ...
    if cfg.project == "supplier-evidence":
        from supplier_evidence.ingestion import enrich_chunks_with_catalog

        chunks = enrich_chunks_with_catalog(
            chunks, Path("data/supplier_evidence/evidence_catalog.json")
        )
