...
.PHONY: install dev run-api run-ui run-chatkit run-supplier-api qdrant-up qdrant-down ingest chat eval run-appv2-backend stop-appv2-backend
...
run-supplier-api:
	 cd app/backend && uv run --active uvicorn app.supplier_main:app --reload --port 8002
